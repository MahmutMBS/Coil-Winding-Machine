...
const float SPOOL_SPEED_LAYER1  = 450.0;  // steps/sec
const float SPOOL_ACCEL_LAYER1  = 250.0;  // steps/sec^2

// SUBSEQUENT layers spool speeds/accels
const float SPOOL_SPEED_OTHERS  = 450.0;  // steps/sec
const float SPOOL_ACCEL_OTHERS  = 250.0;  // steps/sec^2
...
void startNewLayer() {
  ...
  if (currentLayer == 1) {
    ...
    // Reset positions for a clean start
    ...
    // Set targets using the constant BASE_GUIDE_TARGET
    ...
    // For layers 2 and beyond, automatically continue without pausing for adjustment.
    ...
    // Alternate guide direction on each layer
    currentGuideDirection *= -1;
    currentSpoolSteps = SUBSEQUENT_LAYER_SPOOL_STEPS;
    currentEffectiveGuideRatio = effectiveGuideRatio2;

    // Reset positions for the new layer
    spoolMotor.setCurrentPosition(0);
    guideMotor.setCurrentPosition(0);
    startGuidePosition = 0;

    // Set targets for this layer
    currentGuideTarget = BASE_GUIDE_TARGET;
    long targetSpool = currentSpoolSteps;
    long targetGuide = currentGuideDirection * currentGuideTarget;
...
void setup() {
  ...
  Serial.println("'start' to resume, and 'stop' to halt.");
...
void loop() {
  ...
  if (isRunning) {
    ...
    // LAYER COMPLETION: when both motors reach their target, start the next layer automatically
